...
const AllJobs = () => {
  ...
  const url = `/allJobs?page=${currentPage}&size=${itemPerPages}&filter=${filter}&sort=${sort}&search=${search}`;
  useEffect(() => {
    axiosSecure.get(url).then((res) => {
      setJobs(res.data);
    });
  }, [axiosSecure, url, currentPage, itemPerPages, filter, sort, search]);

  const url2 = `/jobsCount?&filter=${filter}&search=${search}`;
  useEffect(() => {
    axiosSecure.get(url2).then((res) => {
      setCount(res.data.count);
    });
  }, [axiosSecure, url2, filter, search]);
